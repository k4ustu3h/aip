...
if [ "$TRAVIS_PULL_REQUEST" = false ]; then
	if [ "$TRAVIS_TAG" ]; then
		...
		for apk in $(find *.apk -type f); do
			...
			upload=`curl "https://github.com/k4ustu3h/aip/releases/${TRAVIS_REPO_SLUG}/releases/${releaseId}/assets?access_token=$ff80ebde0bf2d0a1369e7d5cb812b116fca88efc&name=${apkName}.apk" --header 'Content-Type: application/zip' --upload-file ${apkName}.apk  -X POST`

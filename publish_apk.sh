	...
	if [ "$TRAVIS_TAG" ]; then
		...
		for apk in $(find *.apk -type f); do
			...
			if [ ! -z "$url" -a "$url" != " " -a "$url" != "null" ]; then
                ...
                telegramUrl="https://api.telegram.org/bot931154861:AAGU9U-b_4yu7ih4UGrlhlEf78Ps2V_q2dg/sendMessage?chat_id=@anothericonpack&text=${message}&parse_mode=Markdown&reply_markup=${btns}"
